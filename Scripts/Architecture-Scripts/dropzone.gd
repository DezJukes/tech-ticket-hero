...
func _update_border_visual() -> void:
	...
	# Background
	style.bg_color = Color(0.08, 0.08, 0.08, 0.25)
	...
	if has_component:
		# ACTIVE / FILLED SLOT
		style.border_color = Color(0.0, 0.54, 0.885, 1.0)
...
func _drop_data(_at_position: Vector2, data: Variant) -> void:
	...
	# Remove old component if occupied
	if has_component:
...
func _play_placement_effects(badge: Control) -> void:
	...
	ring_style.border_color = Color(0.0, 0.85, 1.0, 1.0)
	...
	ring_style.corner_radius_top_left = 12
	ring_style.corner_radius_top_right = 12
	ring_style.corner_radius_bottom_left = 12
	ring_style.corner_radius_bottom_right = 12
	...
	particles.color = Color(0.0, 0.85, 1.0, 1.0)
	...
	and event.is_released():
		...
		if has_component:
			...
			for child in get_children():
				...
				and child is not CPUParticles2D:
					child.queue_free()
