...
# ---------------------------------------------------
# NEW: RUNS ONCE WHEN THE INTERFACE LOADS
# ---------------------------------------------------
func _ready() -> void:
...
# ---------------------------------------------------
# Changed the type from 'Control' to 'Node' so it accepts CanvasLayers without crashing
func _popup_modal(modal_node: Node) -> void:
	...
	if modal_node is CanvasLayer:
		...
		# Find the actual green UI panel inside the CanvasLayer
